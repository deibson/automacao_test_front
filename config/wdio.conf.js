...
const { ensureDirSync } = require('fs');
const { ReportAggregator, HtmlReporter } = require('@rpii/wdio-html-reporter')
const log4j = require('log4js')

const argsChromeOptionsDefault = [
    '--start-maximized',
    '--disable-infobars',
    '--no-sandbox',
    '--disable-gpu'
...
if (process.env.HEADLESS === 'true') {
    argsChromeOptionsDefault.push('--headless', '--window-size=1366,368')
}
...
const capabilitiesDefault = {
    ...
    'goog:chromeOptions': {
        args: argsChromeOptionsDefault,
        prefs: {
            'intl.accept_languages': 'pt-BR'
        }
    }
}
const argsReportDefault = browserName => {
    return {
        debug: true,
        outputDir: './reports/html-reports/',
        filename: 'reports.html',
        reportTitle: 'Test Carrinho de produtos Livelo',
...
exports.config = {
    ...
    reporters: [
        ...
        [
            HtmlReporter,
            argsReportDefault(capabilitiesDefault.browserName)
        ]
    ],
    afterTest: function (test) {
        ...
        ensureDirSync('reports/html-reports/screenshots/');
    ...
    onPrepare: function (config, capabilities) {
        let reportAggregator = new ReportAggregator(argsReportDefault(capabilities.browserName))
